...
export interface PiImageRequestContext {
  ...
  /** Request-level image occurrence bound; omission leaves image count unbounded. */
 ...
 * the accumulated image count or base64 payload exceeds its request bounds,
 * the oldest images are replaced by text placeholders until the request fits,
 * so an image-heavy session keeps clearing gateway request-size caps.
 * @param options - the harness request; `options.system`, else a leading `system` message, maps to pi-ai's single `systemPrompt` slot.
