...
const GIT_CONFIG_ENV_PATTERN = /^GIT_CONFIG_(?:COUNT|KEY_\d+|VALUE_\d+)$/i

/**
 * The ambient parent environment minus credential-shaped names, all `DSH_*`
 * names, and command-scoped Git config tuples — the canonical base every
 * harness child starts from. `PATH`,
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (
      ...
      && !GIT_CONFIG_ENV_PATTERN.test(key)
    ) env[key] = value
  }
  return env
}
