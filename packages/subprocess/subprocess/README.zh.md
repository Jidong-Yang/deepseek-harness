...
子进程永远不会隐式继承 harness 的环境秘密：形似凭据的名称、环境中的 `DSH_*` 事实，以及完整的命令级 `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_n`/`GIT_CONFIG_VALUE_n` 元组都会被清除，调用方显式的 `env` 在该清除之后合并。移除完整 Git 元组可防止继承的计数或孤立成员破坏所有 Git 子进程。有意转发的凭据或当前的 `DSH_*` 部署事实仍会到达子进程；显式的 `undefined` 墓碑值则移除一个普通的环境项。
