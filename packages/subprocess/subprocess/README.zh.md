...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 是唯一一份共享的环境清理定义：环境中形似凭据的名称、`DSH_*` 名称和命令级 `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_n`/`GIT_CONFIG_VALUE_n` 元组都会被丢弃，显式 `env` 在清除之后合并。移除完整 Git 元组可防止继承的计数或孤立成员破坏所有 Git 子进程。本地的普通 spawn 与终端 spawn 都应用该定义；拥有自身 spawn 的 SDK 管理传输可直接导入它。
