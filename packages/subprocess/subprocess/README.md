...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` are the one shared scrub definition: ambient credential-shaped names, `DSH_*` names, and command-scoped `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_n`/`GIT_CONFIG_VALUE_n` tuples are dropped, and explicit `env` merges after the scrub. Removing the complete Git tuple prevents an inherited count or orphaned member from corrupting every Git child. The local ordinary and terminal spawns both apply it; SDK-managed transports that own their spawn may import it directly.
