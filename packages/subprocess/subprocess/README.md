...
Children never inherit the harness's ambient secrets: credential-shaped names, ambient `DSH_*` facts, and complete command-scoped `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_n`/`GIT_CONFIG_VALUE_n` tuples are scrubbed, and the caller's explicit `env` merges after that scrub. Removing the complete Git tuple prevents an inherited count or orphaned member from corrupting every Git child. A deliberately forwarded credential or a current `DSH_*` deployment fact still reaches the child; an explicit `undefined` tombstone removes an ordinary ambient entry.
