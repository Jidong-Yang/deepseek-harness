...
- **Image is the only durable binary-result bridge** — PNG, JPEG, WebP, and GIF can enter Native context after exact capability proof. Audio and binary embedded-resource payloads remain execution-local with explicit diagnostics. Inline text resources and resource links are projected as text; the separate MCP Resources capability is still not bridged.
