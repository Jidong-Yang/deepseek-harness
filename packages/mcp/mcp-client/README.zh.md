...
公开工具名称和 JSON 参数保留在 assistant 历史中。规范值始终为程序化调用方与 PTC mode 调用方保留完整的 MCP JSON 块与可选结构化内容；受支持的图片块在确切路由能力得到证明后，按原始顺序与文本一起投影。被拒绝的图片、音频、二进制或格式错误的嵌入资源、资源链接与未知块继续以有界文本诊断可见。内联文本资源会连同 URI 和可选媒体类型进入模型上下文；MCP `isError` 会在图片持久化之前拒绝调用。

#### Token 影响

参数、映射后的文本与持久图片引用保留到压缩（compaction）发生时。内联 MCP base64 只存在于执行局部的规范值中，绝不会复制进会话事件；提供方会从附件存储读取经过校验的字节。音频与二进制嵌入资源载荷不会进入模型上下文；内联文本资源会进入。
...
- **图片是唯一的持久二进制结果桥接**——PNG、JPEG、WebP 与 GIF 在确切能力得到证明后进入 Native 上下文。音频与二进制嵌入资源载荷仍只存在于执行局部并带明确诊断。内联文本资源与资源链接会投影为文本；独立的 MCP Resources 能力尚未桥接。
