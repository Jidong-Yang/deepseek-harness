...
- Native／模型渲染会保留 MCP 块顺序。文本类连续块以换行连接；资源链接保留名称和 URI；内联文本资源保留 URI、可选媒体类型和文本。只有挂载 `ctx.attachments` 且确切调用模型路由明确声明支持图片输入时，受支持的图片才会成为持久核心图片块。整个图片批次会先完成解码与准入，再保存任一成员。格式错误或被拒绝的图片批次、音频、二进制或格式错误的嵌入资源和不受支持的块会成为明确诊断文本，而不会消失。
...
公开工具名称和 JSON 参数会保留在 assistant 历史中。执行局部的规范值始终为程序化调用方和 Code Mode 保留完整 JSON MCP 块及可选结构化内容。在 Native 上下文中，受支持的图片块会在确切路由能力得到证明后，按原始顺序与文本一起持久投影；Code Mode 还会经外层 `run_code` 结果转运这份已经结算的丰富投影，而不改变规范绑定值。被拒绝的图片、音频、二进制或格式错误的嵌入资源、资源链接和未知块会继续以文本诊断可见。内联文本资源会连同 URI 和可选媒体类型进入模型上下文；MCP `isError` 会在持久化图片前拒绝调用。

#### Token 影响

参数、映射后的文本和持久图片引用会保留到压缩（compaction）发生时。内联 MCP base64 只存在于执行局部的规范值中，绝不会复制进会话事件；提供方会从附件存储读取经过校验的字节。音频和二进制嵌入资源载荷仍不会进入模型上下文；内联文本资源会进入。
...
- **图片是唯一的持久二进制结果桥接**：PNG、JPEG、WebP 和 GIF 可以在确切能力得到证明后进入 Native 上下文。音频和二进制嵌入资源载荷仍只存在于执行局部，并配有明确诊断。内联文本资源和资源链接会投影为文本；独立的 MCP Resources 能力仍未桥接。
