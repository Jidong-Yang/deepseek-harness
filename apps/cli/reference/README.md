...
### Windows supervisor startup record

[`scripts/run-windows-web.ts`](../../../scripts/run-windows-web.ts) optionally consumes `DSH_HOST_READY_FILE` and `DSH_HOST_READY_NONCE`, then deletes both before importing the Host or taking its environment snapshot. Both absent preserves ordinary startup. A partial request fails before boot. The nonce is exactly 32 hexadecimal UUID digits; the destination is a local drive-qualified Windows path of at most 240 characters, without traversal, device names, alternate streams or link-shaped ancestors, in a caller-created private, stable attempt directory. Existing destinations are rejected.

The runner subscribes to the existing launcher `AppReady` latch exposed as `profileReady` by the CLI module. Only successful tree activation and launcher setup commit that latch; completing a module import, binding a port, printing a URL, or connecting to Hub does not. Nonfatal MCP startup failures may settle without blocking this signal; their initial attempt still follows the configured plugin startup behavior.

The synchronous publisher writes at most 1 KiB of UTF-8 JSON with exactly `schemaVersion: 1`, `nonce`, `pid`, `state: "ready"`, and canonical UTC `at`. An exclusive temporary file is flushed and published by a same-directory hard link, which atomically refuses to replace a raced file or link. The local filesystem must support hard links; validation and publication failures produce static diagnostics without request values. The supervising process owns the directory ACL, unique attempt nonce, record validation, cleanup and process/Job liveness. This record describes completed startup, not ongoing health; its Host PID is diagnostic, not adoption or termination authority. No Session data or telemetry is included.

An optional `DSH_HOST_LOCAL_LINK_PIPE` requests a separate local browser-link handoff. The runner removes it before Host environment capture; it requires the startup nonce and a pipe name `ira-dsh-link-` followed by 32 lowercase hexadecimal digits. After AppReady and successful readiness publication, the CLI's `profileLocalUrl` getter supplies the existing authenticated `127.0.0.1` root URL. A one-shot Node sender writes at most 2048 UTF-8 bytes (`schemaVersion`, `nonce`, `pid`, `url`) to the caller-owned Windows current-user pipe with a two-second deadline. No authenticated URL is added to the readiness file or diagnostic output. The local supervisor validates nonce/PID/port before displaying the URL; it must not upload or persist that console. Missing/unreachable handoff does not revoke Host readiness. See [`windows-local-link.ts`](../../../scripts/windows-local-link.ts).
