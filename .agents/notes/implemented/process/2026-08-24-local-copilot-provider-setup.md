...
This fork routinely uses the separate `C:\DSH\copilot-dsh-provider` checkout as a local custom model provider. A new Windows development machine otherwise requires a second clone, two package managers, dependency installation, provider Device Flow authentication, two long-running processes, route configuration, model synchronization, placeholder authentication, and verification. Reconstructing that sequence in each session is error-prone. The deployment is still machine-specific and relies on undocumented GitHub Copilot inference endpoints, so placing it in every agent's default project context or in shipped dsh profiles would present a local convention as a product requirement.

## Decision

The project-level [`dsh-copilot-provider-setup`](../../../skills/dsh-copilot-provider-setup/SKILL.md) skill owns the complete cross-repository bootstrap and recurring integration procedure. Starting from this fork's checkout, it prepares the provider and invokes the Harness's idempotent Windows deployment path, while the root `setup.ps1` owns Harness dependency installation, build, bridge publication, and its Task Scheduler lifecycle ([decision](2026-09-01-windows-source-deployment-task.md)). The skill reuses valid existing installations, checkouts, authentication, builds, and processes, and never rewrites an existing worktree to force it into the convention.

The conventional deployment uses provider repository `https://github.com/Jidong-Yang/copilot-dsh-provider.git`. The `copilot-proxy` route serves `openai-responses` from `http://127.0.0.1:4141/responses/v1`, while `copilot-chat` serves `openai-completions` from `http://127.0.0.1:4141/chat/v1`. The skill reads the provider's current README and both live protocol-specific model listings before writing configuration, preserves unrelated settings, and synchronizes only those two routes. It carries a non-secret placeholder authorization because the provider ignores inbound credentials; GitHub and Copilot tokens never enter dsh settings. Each route permits one Harness retry for transient or empty responses because the provider already performs its own single authentication recovery.

The provider's live catalogs own model membership and disclosed metadata. The setup preserves capacities, supported input modalities, and exact reasoning-level wire spellings. Its diagnostics interpret the provider's safe health status: `checking` waits, `upstream-unavailable` backs off without Device Flow, and `reauth-required` distinguishes rejected GitHub credentials from rejected Copilot access. A refused loopback connection points to the `Copilot DSH Provider` Task Scheduler task rather than model configuration. Verification covers provider readiness, live dsh registration, both catalogs, bounded requests through each populated protocol route, advertised image and reasoning inputs, and both loopback listeners.
